...
#### DATA PREPARATION ####
## IMPORT TPM DATA
setwd("~/Dropbox/RNA seq data/FuniNKT DysiNKT")
tpm.dat <- read.csv("2Dys 1Fun iNKT TPM.csv")
head(tpm.dat)
...
#FuniNKT
FuniNKT.dat <- tpm.dat_ins[,-c(4:6)]

## FILTER GENES WITH ZERO VALUE ACROSS ALL SAMPLES OUT
FuniNKT.dat_cut.zero <- FuniNKT.dat[-which(apply(FuniNKT.dat[, -c(1:3)], 
                                                 1, sum) == 0), ]
boxplot(log2(FuniNKT.dat_cut.zero[, -c(1:3)] + 0.25))
...
for (gene.name in FuniNKT.dat_cut.zero$Gene) {
  tmp <- as.numeric(FuniNKT.dat_cut.zero[
    FuniNKT.dat_cut.zero$Gene == gene.name, -c(1:3)])
...
samp.cutoff <- (ncol(FuniNKT.dat_cut.zero)-3)*.5 # 50% OF NUMBER OF SAMPLES
FuniNKT.dat_cut.low <- FuniNKT.dat_cut.zero[which(num.collect > samp.cutoff), ]
boxplot(log2(FuniNKT.dat_cut.low[, -c(1:3)] + 0.25))
write.csv(FuniNKT.dat_cut.low, "FuniNKT_TPM_cutoff.csv")
...
#Import data
counts <- read.csv("FuniNKT_TPM_cutoff.csv", row.names = 3)
counts <- counts[,-c(1:3)]

metadata <- read.csv("FuniNKT_meta.csv",header = T)
...
contr <- makeContrasts(ConditionFuniNKT - ConditioniNKT, levels = colnames(coef(fit)))
...
contrast.matrix <- makeContrasts(ConditionFuniNKT - ConditioniNKT, levels=colnames(coef(fit)))
...
write.table(filtered_data, file = "FuniNKTvsiNKT.txt", row.names = F, sep = "\t", quote = F)


#Filter genes across FuniNKT and DysiNKT
FunDysiNKT.dat <- tpm.dat_ins[,-c(8:9)]

## FILTER GENES WITH ZERO VALUE ACROSS ALL SAMPLES OUT
FunDysiNKT.dat_cut.zero <- FunDysiNKT.dat[-which(apply(FunDysiNKT.dat[, -c(1:3)], 
                                                 1, sum) == 0), ]
boxplot(log2(FunDysiNKT.dat_cut.zero[, -c(1:3)] + 0.25))
...
num.collect <- vector() # CREATE A VECTOR FOR COLLECTING THE NUMBER OF SAMPLES WHICH HIT THE CONDITION --> EXPRESSION VALUE > QUANTILE VALUE IN EACH GENE ACROSS ALL SAMPLES
# THIS FOR LOOP WILL BE PROCESSING FOR A WHILE
for (gene.name in FunDysiNKT.dat_cut.zero$Gene) {
  tmp <- as.numeric(FunDysiNKT.dat_cut.zero[
    FunDysiNKT.dat_cut.zero$Gene == gene.name, -c(1:3)])
  num.collect <- append(num.collect, length(which(tmp > quantile(tmp, qt.cutoff))))
}
table(num.collect) # CHECK THE DISTRIBUTION OF SAMPLES

## A SCORE WAS CONSTRUCTED FOR EACH GENE BY COUNTING THE NUMBER OF SAMPLES WITH EXPRESSION VALUES BELOW THE 25TH GENE PERCENTILE 
samp.cutoff <- (ncol(FunDysiNKT.dat_cut.zero)-3)*.5 # 50% OF NUMBER OF SAMPLES
FunDysiNKT.dat_cut.low <- FunDysiNKT.dat_cut.zero[which(num.collect > samp.cutoff), ]
boxplot(log2(FunDysiNKT.dat_cut.low[, -c(1:3)] + 0.25))
write.csv(FunDysiNKT.dat_cut.low, "FunDysiNKTiNKT_TPM_cutoff.csv")


library(dplyr)
#load tpm data
tpm.data <- read.csv("FunDysiNKTiNKT_TPM_cutoff.csv")
...
write.csv(merged_data, file = "FuniNKTvsiNKT TPM.csv")

################

#DysiNKT
DysiNKT.dat <- tpm.dat_ins[,-7]
...
metadata <- read.csv("DysiNKT_meta.csv",header = T)
identical(metadata$X, colnames(counts))
...
library(dplyr)
#load tpm data
tpm.data <- read.csv("FunDysiNKT_TPM_cutoff.csv")
# Merge the data frames based on SYMBOL
merged_data <- merge(filtered_data, tpm.data, by = "SYMBOL")
...
Hallmarks <- read.csv("MSigDB_Hallmark.csv", header = TRUE, 
...
ggplot(df, aes(x= Combined.Score, y=Term, size= Overlap, color= P.value)) + geom_point() +
  theme_classic() +
  scale_color_gradient(low = "red", high = "black", breaks = c(0.004,0.01,0.04))+ 
  labs(x = "Combined Score",y = "MSigDB_Hallmark 2020",size = "Overlap score",color = "P-value")

#Fig. 2F
#Merge PLSR data with TPM
DGE.tpm <- read.csv("Top500 FunDysiNKT PLSR VIP.csv")
#load tpm data
tpm.data <- read.csv("FunDysiNKTiNKT_TPM_cutoff.csv")
# Merge the data frames based on SYMBOL
...
write.csv(merged_data, file = "FunDysiNKT PLSR VIP TPM.csv")
...
VIP.tpm <- read.csv("FunDysiNKT PLSR VIP TPM.csv", row.names = 2)
...
write.csv(tpm.dat_median, "FuniNKTvsDysiNKT_normalized.csv")

#Plot Fig. 2F in graphpad prism using FuniNKTvsDysiNKT_normalized, VIP score, and gene involved in each pathway
